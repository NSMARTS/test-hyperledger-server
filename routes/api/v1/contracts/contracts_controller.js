...
exports.getPdf = async (req, res) => {
    ...
    try {
        ...
        const foundContract = await dbModels.Contract.findById(id).lean();
...
exports.getContractById = async (req, res) => {
    ...
    try {
        ...
        // 트랜잭션 시작
        await session.startTransaction();
        const foundContract = await dbModels.Contract.findById(id, { location: 0 })
            .populate({
                path: "writer",
...
exports.verifyContracts = async (req, res) => {
    ...
    try {
        ...
        const network = await gateway.getNetwork("contractchannel");
        const contract = network.getContract("contract");
        const resultBuffer = await contract.submitTransaction("ReadContractById", id);
        ...
        if (!verifyResult) {
            ...
            return res.status(400).json({
                message: "Verification Failed: The document you have uploaded does not match the expected contract or certificate.",
            });
        }
